        ...
        ({
            ...
            project = {
                ...
                protectHomeShellCode = ''
                    # 
                    # find the VIRKSHOP_core
                    # 
                    path_to_VIRKSHOP_core=""
                    file_name="settings/VIRKSHOP_core"
                    ...
                    do
                        ...
                        then
                            path_to_VIRKSHOP_core="$folder_to_look_in/$file_name"
                            break
                        else
                    ...
                    if [ -z "$path_to_VIRKSHOP_core" ]
                    then
                        #
                        # what to do if file never found
                        #
                        echo "Im part of parse_dependencies.nix, a script running with a pwd of:$PWD"
                        echo "Im looking for settings/VIRKSHOP_core in a parent folder"
                        echo "Im exiting now because I wasnt able to find it"
                        echo "thats all the information I have"
                        exit
                    fi
                    export VIRKSHOP_NEXT_RUN_DONT_DO_MANUAL_START="true"
                    . "$path_to_VIRKSHOP_core"
                    
                    # ensure that the folder exists
                    mkdir -p "$(dirname "$__VIRKSHOP_NIX_PATH_EXPORT_FILE")"
                    echo ${main.escapeShellArg (packagePathsAsJson)} > "$__VIRKSHOP_NIX_PATH_EXPORT_FILE"
                    
                    if [ -n "$VIRKSHOP_HOME" ]
                    then
                        # we don't want to give nix or other apps our home folder
                        if ! [ "$HOME" = "$VIRKSHOP_HOME" ]
                        then
                            if [ "$VIRKSHOP_DEBUG" = "true" ]; then
                                echo "replacing: HOME with VIRKSHOP_HOME"
                            fi
                            mkdir -p "$VIRKSHOP_HOME/.cache/"
                            ln -s "$HOME/.cache/nix/" "$VIRKSHOP_HOME/.cache/" &>/dev/null
                            
                            # so make the home folder the same as the project folder
                            export HOME="$VIRKSHOP_HOME"
