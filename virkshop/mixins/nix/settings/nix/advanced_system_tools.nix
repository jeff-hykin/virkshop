...
let        
    # 
    # load most things from system_tools.toml
    # 
    main = (builtins.import ''${(main.getEnv "VIRKSHOP_FOLDER" )}/mixins/nix/internals/parse_dependencies.nix'');
    ...
    main.packages.mkShell {
        ...
        shellHook = ''
            ...
            if [ "$VIRKSHOP_DEBUG" = "on" ]; then
                echo "starting: 'shellHook' inside the 'advanced_system_tools.nix' file"
            fi
            ${linuxOnly.shellCode}
            ${macOnly.shellCode}
            ...
            if [ "$VIRKSHOP_DEBUG" = "on" ]; then
                echo "starting: 'shellHook' inside the 'advanced_system_tools.nix' file"
                echo ""
                echo "Tools/Commands mentioned in 'system_tools.toml' are now available/installed"
            fi
        '';
    }
