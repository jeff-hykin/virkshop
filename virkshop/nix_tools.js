...
import { OperatingSystem } from "https://deno.land/x/quickr@0.4.6/main/operating_system.js"
...
export async function sourceNixSetup() {
    // 
    // add nix certificate file
    // 
        // make sure MacOS has cert files
        const certFilePath = FileSystem.makeAbsolutePath("/etc/ssl/certs/ca-certificates.crt")
        ...
        if (Console.env.NIX_SSL_CERT_FILE) {
            const profileFolderInfo = await FileSystem.info(`/nix/var/nix/profiles/default/etc/`)
            if (profileFolderInfo.isFolder) {
                const nixCertPath = `/nix/var/nix/profiles/default/etc/ssl/certs/ca-bundle.crt`
                const nixCertPathInfo = await FileSystem.info(nixCertPath)
...
// 
// 
export async function enterVirkshop() {
    // summary
